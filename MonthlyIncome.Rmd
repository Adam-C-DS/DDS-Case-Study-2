...
```
```{r}
cs2.reduced <- cs2.data %>% dplyr::select(JobLevel, JobRole, TotalWorkingYears, YearsWithCurrManager, YearsSinceLastPromotion,
                                          BusinessTravel, MonthlyRate,Gender, MonthlyIncome)
...
```{r}
Model.MIMin <- lm(MonthlyIncome ~ JobLevel + JobRole + TotalWorkingYears, data = cs2.data[,-34])
vif(Model.MIMin)
summary(Model.MIMin)
...
```{r}
# Crazy Model
Model.MICrazy <- lm(MonthlyIncome ~ ,data = cs2.data[,-c(1,34)])
```
...
for(i in iterations){
  ...
  Model.MIMin <- lm(MonthlyIncome ~ JobLevel + JobRole + TotalWorkingYears, data = train.mi[,-c(1,34)])
...
Model.Average.RMSE = cbind(rmseStep, rmseFull, rmseMin)
